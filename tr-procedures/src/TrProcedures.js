...
export class TrProcedures extends ApiFunctions(CredDialog) {
    ...
    resetView() {
        let findProc = JSON.parse(
            sessionStorage.getItem("userSession")
        ).procedures_list.procedures.filter(
            (m) => m.procInstanceName == this.procName
        );
        ...
        if (ProceduresModel[this.procName] === undefined) {
            alert(
                "Model for procedure " +
                    this.procName +
                    " not found in the ProceduresModel (probably not load in local)"
            );
            return;
        }
        this.viewModelFromProcModel =
            ProceduresModel[this.procName][this.viewName];
        if (this.viewModelFromProcModel === undefined) {
            alert(
                "Not found any window called " +
                    this.viewName +
                    " in the Backend Proc Model for procedure " +
                    this.procName
            );
            return;
        }
        if (this.viewModelFromProcModel.component === undefined) {
            alert(
                "The window called " +
                    this.viewName +
                    " has no component specified in the Backend Proc Model for procedure " +
                    this.procName
            );
            return;
        }
        ...
        console.log(
            "resetView",
            "component",
            this.viewModelFromProcModel.component
        );
        ...
        switch (this.viewModelFromProcModel.component) {
            ...
            case "PrototypeElementsViewMain":
                ...
                import(
                    "./components/0PrototypeElements/prototype-elements-view-main"
                );
                return;
            case "TreeView":
                this.windowOpenable = true;
            ...
            default:
                alert(
                    "In TrProcedures, not found component " +
                        this.viewModelFromProcModel.component
                );
                return;
        }
    ...
    async authorized() {
        ...
        let procList = JSON.parse(sessionStorage.getItem("userSession"))
            .procedures_list.procedures;

        if (this.procName === undefined || procList === undefined) {
            return;
        }

        let anyAccess = procList.filter(
            (p) => p.procInstanceName == this.procName
        );
        if (anyAccess.length) {
            let defView = anyAccess[0].new_definition.filter(
                (d) => d.lp_frontend_page_name == this.viewName
            );
            if (defView.length == 0) {
                defView = anyAccess[0].icons_up.filter(
                    (d) => d.lp_frontend_page_name == this.viewName
                );
            }
            if (defView.length == 0) {
                defView = anyAccess[0].icons_down.filter(
                    (d) => d.lp_frontend_page_name == this.viewName
                );
            }
            if (defView.length) {
                ...
                if (defView[0].icons) {
                    let sopIcon = defView[0].icons.filter(
                        (i) => i.name == this.filterName
                    );
            ...
            if (!this.sopsPassed) {
                if (
                    anyAccess.length &&
                    anyAccess[0].userSopMode !== undefined &&
                    anyAccess[0].userSopMode
                        .toString()
                        .toUpperCase()
                        .includes("DISAB")
                ) {
                    ...
                    if (
                        ...
                        this.windowOpenable =
                            anyAccess[0].windowOpenableWhenNotSopCertifiedUserSopCertification.toLowerCase();
                    }
                }
            }
                ...
                this.dispatchEvent(
                    new CustomEvent("error", {
                        detail: {
                            is_error: true,
                            message_en:
                                "Window cannot be open due to pending linked SOP certifications",
                            message_es:
                                "La ventana no se puede abrir porque hay SOPs vinculados pendientes de certificación",
                        },
                        bubbles: true,
                        composed: true,
                    })
            ...
            if (anyAccess.length) {
                if (this.tabs) {
                    this.tabsComposition.updateComplete.then(() => {
                        this.tabsComposition.model =
                            ProceduresModel[this.procName][
                                this.viewName
                            ].tabs[0];
                    });
                } else {
    ...
    ownComponents() {
        ...
        if (
            ...
            this.viewModelFromProcModel.component !== undefined
        ) {
            //console.log('ownComponents', 'component', this.viewModelFromProcModel.component, 'viewName', this.viewName)
        }
        return html`
            ${this.viewModelFromProcModel !== undefined &&
            this.viewModelFromProcModel.component == "Browser"
                ? html`
                      <browser-view
                          .config=${this.config}
                          .desktop=${this.desktop}
                          .lang=${this.lang}
                          .model=${ProceduresModel[this.procName]}
                          .procName=${this.procName}
                      ></browser-view>
                  `
                : html``}
            ${this.viewModelFromProcModel &&
            this.viewModelFromProcModel.component == "DataMining"
                ? html`
                      <datamining-mainview
                          .config=${this.config}
                          .desktop=${this.desktop}
                ...
                : html``}
            ${this.viewModelFromProcModel &&
            this.viewModelFromProcModel.component == "Flowchart"
                ? html`
                      <flow-chart
                          .config=${this.config}
                          .desktop=${this.desktop}
                ...
                : html``}
            ${this.viewModelFromProcModel &&
            this.viewModelFromProcModel.component == "ModuleEnvMonitHomeAir"
                ? html`
                      <home-air
                          .config=${this.config}
                          .desktop=${this.desktop}
            ...
            ${this.viewModelFromProcModel &&
            this.viewModelFromProcModel.component ==
                "ModuleEnvMonitCultureMedium"
                ? html` <culture-medium></culture-medium> `
                : html``}
            ${this.viewModelFromProcModel &&
            this.viewModelFromProcModel.component == "TreeView"
                ? html` <tree-view-main></tree-view-main> `
                : html``}
            ${this.viewModelFromProcModel &&
            this.viewModelFromProcModel.component == "PrototypeElementsViewMain"
                ? html`
                      <prototype-elements-view-main></prototype-elements-view-main>
                  `
                : html``}
            ${this.viewModelFromProcModel &&
            this.viewModelFromProcModel.component == "ModuleEnvMonitHomeWater"
                ? html`
                      <home-water
                          .config=${this.config}
                          .desktop=${this.desktop}
                ...
                : html``}
            ${this.viewModelFromProcModel &&
            this.viewModelFromProcModel.component == "ModuleSampleLogSample"
                ? html`
                ...
                : html``}
            ${this.viewModelFromProcModel &&
            this.viewModelFromProcModel.component == "ModuleEnvMonitProgramProc"
                ? html`
                      <program-proc
                          .windowOpenable=${this.windowOpenable}
                          .sopsPassed=${this.sopsPassed}
            ...
            ${this.viewModelFromProcModel &&
            this.viewModelFromProcModel.component ==
                "ModuleEnvMonitSampleIncubation"
                ? html`
                      <sample-incubation-view
                          .windowOpenable=${this.windowOpenable}
                          .sopsPassed=${this.sopsPassed}
                          .lang=${this.lang}
                          .procInstanceName=${this.procName}
                ...
                : nothing}
            ${this.viewModelFromProcModel &&
            this.viewModelFromProcModel.component == "ObjectByTabs"
                ? html`
                      <object-by-tabs
                          .windowOpenable=${this.windowOpenable}
                          .sopsPassed=${this.sopsPassed}
                ...
                : nothing}
            ${this.viewModelFromProcModel &&
            this.viewModelFromProcModel.component == "Dashboard"
                ? html`
                      <dynamic-dashboard
                          .params=${this.viewModelFromProcModel.data}
                          .lang=${this.lang}
                          .windowOpenable=${this.windowOpenable}
                          .sopsPassed=${this.sopsPassed}
                          .procInstanceName=${this.procName}
                ...
                : nothing}
            ${this.viewModelFromProcModel &&
            this.viewModelFromProcModel.component == "CalendarData"
                ? html`
                      <calendar-data
                          .windowOpenable=${this.windowOpenable}
                          .sopsPassed=${this.sopsPassed}
                ...
                : nothing}
            ${this.viewModelFromProcModel &&
            this.viewModelFromProcModel.component == "ModuleGenomaProjectWindow"
                ? html`
                      <genoma-project
                          .windowOpenable=${this.windowOpenable}
                          .sopsPassed=${this.sopsPassed}
    ...
    render() {
        ...
        return html`
            ${this.windowOpenable == "yes"
                ? html`
                      ${this.viewModelFromProcModel &&
                      this.viewModelFromProcModel.component !== null &&
                      this.viewModelFromProcModel.component !== undefined &&
                      this.viewModelFromProcModel.component.toLowerCase() ==
                          "tablewithbuttons"
                          ? html`
                                <grid-with-buttons
                                    id="gridwithbuttons"
                          ...
                          : html`
                                ...
                                this.viewModelFromProcModel.component.toLowerCase() ==
                                    ...
                                    ? html`
                                          <tabs-main-view
                                              id="tabsmainview"
                                              .tabsMainViewModelFromProcModel=${this
                                                  .viewModelFromProcModel}
                                              viewName=${this.viewName}
                                              filterName=${this.filterName}
                                              procInstanceName=${this.procName}
    ...
    get GridWithButtons() {
        return this.shadowRoot.querySelector(
            "grid-with-buttons#gridwithbuttons"
        );
    }
}
